...
class etpDatabase:
    ...
    def resolveNeeded(self, needed, elfclass = -1):
        ...
        self.cursor.execute('SELECT idpackage,file FROM content WHERE content.file IN (?) and (?) in (select library from neededreference where library = (?));', (mypaths,needed,needed,))
