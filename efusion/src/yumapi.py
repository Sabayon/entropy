...
class YumexYumHandler(yum.YumBase,YumexPackages):
    def __init__(self,recent,settings,progress,mainwin,parser):
        ...
        #self.repos.setInterruptCallback(self._interrupt_callback)
...
#    def _setupFileLog( self, logfile='/var/log/yum.log' ):  
#        if self.conf.uid == 0:
#            try:
#                filelogger = logging.getLogger( "yumex.filelogging" )
...
#                
#            except Exception, e:
#                logging.getLogger( "yumex" ).critical( 'Cannot open logfile %s', logfile )
    ...
    def _downloadPackages(self):
        ...
        try:
            ...
            #self.yumex_logger.error(errmsg)
            raise yum.Errors.YumBaseError( errstring )
...
#        lvl = yum.logginglevels.logLevelFromDebugLevel( debuglevel )
#        logging.getLogger( 'yum' ).setLevel( lvl )
