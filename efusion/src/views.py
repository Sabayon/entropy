    ...
    def populate(self,data,tree=False):
        ...
        if tree:
            ...
            for key in keys:
                lst = data[key]
                lst.sort()
                keyroot = self.model.append(None,[key,''])
                for el in lst:
                    full = '%s/%s' % (key,el)
                    self.model.append(keyroot,[el,full])
        else:
            for el in data:
    ...
    def get_data_text( self, column, cell, model, iter,property ):
        ...
        if obj:
            try:
                ...
                #   that same thread.
                #   This occurs sometimes because the view updates are
                #   triggered by the gtk event processor running in a thread
                #   It should not break something, because we are only reading.
                pass 
...
class YumexGroupsCategoryView:
    ...
    def populate(self,data):
        ...
        for el in data:
            name,id = el
            self.model.append([name,id])

class YumexGroupsView:
    def __init__( self, treeview,qview):
        self.view = treeview
        self.model = self.setup_view()
        self.queue = qview.queue
    ...
    def setup_view( self ):
        """ Setup Group View  """
        model = gtk.ListStore(gobject.TYPE_BOOLEAN, # Installed
        ...
        cell2 = gtk.CellRendererPixbuf()    # Queue Status
        column2 = gtk.TreeViewColumn( "", cell2 )
        column2.set_cell_data_func( cell2, self.new_pixbuf )
        column2.set_sizing( gtk.TREE_VIEW_COLUMN_FIXED )
        column2.set_fixed_width( 20 )
        ...
        column3= gtk.TreeViewColumn( _( "Groups" ), cell3, markup=1 )
    ...
    def _updatePackages(self,id,add,action):
        ...
        pkgs = self.yumbase.getByGroup(grp,['m','d'])
    ...
    def populate(self,data):
        ...
        for el in data:
            name,id,inst = el
            self.model.append([inst,name,id,False])
    ...
    def new_pixbuf( self, column, cell, model, iter ):
        ...
        if action:            
            ...
            cell.set_property( 'visible', True )
            cell.set_property( 'stock-id', icon )
