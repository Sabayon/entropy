...
class YumexController(Controller):
    ...
    def on_GrpCategories_cursor_changed(self, widget):
        """ Handle selection of row in Group Category view  """
        ( model, iterator ) = widget.get_selection().get_selected()
        if model != None and iterator != None:
            id = model.get_value( iterator, 1 )
            self.populateGroups(id)

    def on_GrpGroups_cursor_changed(self, widget):
        """ Handle selection of row in Group  view  """
        ( model, iterator ) = widget.get_selection().get_selected()
        if model != None and iterator != None:
...
# Menu Handlers
    ...
    def on_HelpAbout( self, widget ):
        about = AboutDialog(const.PIXMAPS_PATH+'/yumex-about.png',const.CREDITS)
        about.show()
...
class YumexApplication(YumexController,YumexGUI):    
    ...
    def __init__(self):
        ...
        self.grpGroups.yumbase = self.yumbase
    ...
    def populateGroupCategories(self):
        catkeys = [ (c.name,c.categoryid) for c in self.yumbase.comps.categories]
        catkeys.sort()
        self.grpCategories.populate(catkeys)
        self.grpGroups.model.clear()
    ...
    def populateGroupPackages(self,id):
        ...
        pkgs = self.yumbase.getByGroup(grp,['m','d','o'])
        pkgs.sort()
        self.grpPackages.store.clear()
